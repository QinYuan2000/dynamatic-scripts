module {
  ...
  ^bb1(%1: index, %2: i32):  // 2 preds: ^bb0, ^bb1
    ...
    %4 = memref.load %arg1[%1] {name = #handshake.name<"load3">} : memref<1000xi32>
    %5 = arith.muli %arg3, %4 {name = #handshake.name<"muli1">} : i32
    %6 = arith.addi %3, %5 {name = #handshake.name<"addi0">} : i32
    memref.store %6, %arg0[%1] {name = #handshake.name<"store1">} : memref<1000xi32>
    %7 = arith.addi %1, %c1 {name = #handshake.name<"addi1">} : index
